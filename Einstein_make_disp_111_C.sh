...
for i in $list; do 
  cd $i
  lineold=$(sed -n  '40,40'p POSCARold); 
  var1=$(sed -n  '40,40'p POSCARold | awk '{print $1}'); 
  var2=$(sed -n '42,42'p POSCARold | awk '{print $1}');
  var3=$( echo $var2+$var1*$scale111-$var2*$scale111 | bc -l);
  var4=$( echo $var1*$scale111-$var2*$scale111 | bc -l);
  linenew=$(echo "" $var3 "" $var4 "" $var4)
